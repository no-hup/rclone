...
func (b *s3Backend) PutObject(
	...
	// Upload via a temporary object renamed into place if needed
	fsys := _vfs.Fs()
	tmpFp := fp
	if (fsys.Features().PartialUploads || _vfs.Opt.CacheMode >= vfscommon.CacheModeWrites) && operations.CanServerSideMove(fsys) {
