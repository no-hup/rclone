...
// TestPutObjectFailureCacheModeWrites checks the failed PUT semantics with
// --vfs-cache-mode writes: the truncated file the failed upload leaves in
// the cache must be discarded, not written back over the object at the key.
func TestPutObjectFailureCacheModeWrites(t *testing.T) {
	vfsOpt := vfscommon.Opt
	vfsOpt.CacheMode = vfscommon.CacheModeWrites
	vfsOpt.WriteBack = fs.Duration(100 * time.Millisecond)
	b, f, bucket := newPutTestBackend(t, "", &vfsOpt)
	...
	// Wait out several write-back intervals to catch the truncated data
	// being written back before checking the object is untouched.
